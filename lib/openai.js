...
class CostTracker {
  ...
  calculateCost(inputTokens, outputTokens, model = 'gpt-4o') {
    const rates = {
      'gpt-3.5-turbo': { input: 0.0015, output: 0.002 }, // per 1K tokens (updated rates)
      'gpt-4': { input: 0.03, output: 0.06 },
      'gpt-4-turbo': { input: 0.01, output: 0.03 },
      'gpt-4o': { input: 0.005, output: 0.015 } // per 1K tokens for GPT-4o
    };
...
Focus on helping English speakers pronounce Albanian sounds correctly.`
  ...
  async makeStructuredRequest(messages, schema, operationName = 'structured') {
    // Check budget before making request
    this.costTracker.checkBudget(this.config.maxDailyCost);
    ...
    return this.executeWithRetry(async () => {
      ...
      // No need for JSON parsing cleanup - it's guaranteed valid JSON
      return {
